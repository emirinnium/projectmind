...
export function registerInvocationMetricsTool(server: McpServer, _deps: McpDependencies): void {
  server.registerTool(
    'get_invocation_metrics',
    {
      ...
      description:
        'Return in-process MCP latency, output-byte and estimated-token aggregates. Metrics are observational and never treated as review correctness or confidence.',
      inputSchema: {
          ...
          text: JSON.stringify(
            {
              ...
              limitations: [
                'Metrics cover this server process only; they are not a cross-machine benchmark.',
              ],
