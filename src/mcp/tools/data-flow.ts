...
export function registerDataFlowTools(server: McpServer, deps: McpDependencies): void {
  ...
  server.registerTool(
    'record_data_flow',
    {
      ...
      description:
        'Record a data-flow edge between resources or functions for taint tracking. Use this when source code analysis identifies how data moves between sinks and sources.',
      inputSchema: {
        fromQualifiedName: z
          .string()
          .describe('Qualified name of the source resource (e.g. fs.readFile("./input.txt"))'),
        fromKind: z
          .enum(['FILE', 'NETWORK', 'DATABASE', 'ENV', 'STDIN', 'STDOUT', 'STDERR', 'SOCKET'])
          .describe('Kind of the source resource'),
        fromIdentity: z
          .string()
          .describe('Identity string of the source resource (file path, URL, env var name, etc.)'),
        toQualifiedName: z.string().describe('Qualified name of the target resource or function'),
        toKind: z
          .enum(['FILE', 'NETWORK', 'DATABASE', 'ENV', 'STDIN', 'STDOUT', 'STDERR', 'SOCKET'])
          .describe('Kind of the target resource'),
        toIdentity: z.string().describe('Identity string of the target resource'),
        kind: z
          .enum(['resource', 'arg', 'return'])
          .describe('How the data flows: direct resource, function argument, or return value'),
        via: z
          .string()
          .optional()
          .describe('Optional intermediate (function name, variable, etc.)'),
  ...
  server.registerTool(
    'get_data_flows',
    {
      ...
      description:
        'Get all recorded data flows for the current project. Shows how resources and functions are connected via taint edges.',
      inputSchema: {},
    },
    async () => {
      try {
  ...
  server.registerTool(
    'get_resource_flows',
    {
      ...
      description:
        'Get all data flows (incoming and outgoing) for a specific resource. Useful for understanding how a file, network endpoint, or env var participates in data flow.',
      inputSchema: {
