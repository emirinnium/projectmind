...
/**
 * Precomputed sin lookup table for fast embedding generation.
 * The hash-to-sin mapping is deterministic, so we cache sin values
 * indexed by (hash % TABLE_SIZE + d * STEP) % TABLE_SIZE to avoid
 * recomputing Math.sin on every token*dimension pair.
 */
...
/** Memoize the full hash→sin-vector mapping so repeated calls for the same
 * token string are essentially free (O(1) map lookup). */
...
/** Clear the token-vector cache. Useful in long-running sessions to reclaim memory. */
