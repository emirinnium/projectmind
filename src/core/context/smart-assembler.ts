...
export interface SmartContextItem {
  path: string;
  score: number;
  reasons: string[];
}
...
export function assembleSmartContext(
  kg: KnowledgeGraph,
  options: {
    fileId: number;
    relativePath: string;
  ...
  const consider = (info: FileInfo, points: number, reason: string): void => {
  ...
  const items: SmartContextItem[] = ranked.slice(0, cap).map(({ path, score, reasons }) => ({ path, score, reasons }));
