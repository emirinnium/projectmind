...
export class ProjectScanner {
  ...
  async scanProject(rootPath?: string): Promise<{ scanned: number; errors: number }> {
    const profile = await this.scanProjectWithProfile(rootPath);
  ...
  async scanProjectWithProfile(rootPath?: string): Promise<ScanProfile> {
    ...
    try {
      ...
      errorDetails.push(`Transaction failed: ${e instanceof Error ? e.message : String(e)}`);
      throw e;
    }
    ...
    return {
      ...
      errorFiles: errors,
      durationMs,
      filesPerSecond: Math.round(scanned / (durationMs / 1000)),
