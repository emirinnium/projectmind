          ...
          await withService(['scale'], async (_ctx, services) => {
              ...
              const content = JSON.stringify(
                {
                  churnData,
                  highRisk,
                  summary: { totalFiles: churnData.length, highRiskCount: highRisk.length },
                },
                null,
                2,
              );
            ...
            if (opts.by === 'file') {
              ...
              for (const item of sorted.slice(0, 30)) {
                const riskIcon =
                  item.riskScore >= riskThreshold
                    ? '🔴'
                    : item.riskScore >= riskThreshold * 0.5
                      ? '🟡'
                      : '🟢';
                const churnIcon = item.churnCount > 10 ? '🔄' : item.churnCount > 5 ? '🔁' : '➡️';
                output.kv(
                  `${riskIcon} ${churnIcon} ${item.path}`,
                  `Risk: ${(item.riskScore * 100).toFixed(1)}% | Churn: ${item.churnCount} | Load: ${item.cognitiveLoad.toFixed(3)} | Authors: ${item.authors.join(', ')}`,
                );
              }
            } else if (opts.by === 'author') {
              const byAuthor = new Map<
                string,
                { files: number; totalRisk: number; totalChurn: number }
              >();
              ...
              for (const [author, data] of byAuthor) {
                ...
                const icon =
                  avgRisk >= riskThreshold ? '🔴' : avgRisk >= riskThreshold * 0.5 ? '🟡' : '🟢';
                output.kv(
                  `${icon} ${author}`,
                  `Files: ${data.files} | Avg Risk: ${(avgRisk * 100).toFixed(1)}% | Total Churn: ${data.totalChurn}`,
                );
              }
...
function calculateChurnFromSessions(
  files: ScaleReport['modules'][number]['files'],
  projectRoot: string,
  sinceDays: number,
): ChurnResult[] {
...
function generateHtmlChurn(
  churnData: ChurnResult[],
  highRisk: ChurnResult[],
  threshold: number,
): string {
  const rows = churnData
    .map(
      (item) => `
