...
export function createSecretsLifeCommand(): Command {
  const secretsCmd = new Command('secrets-life')
    ...
    .action(asyncHandler(async (opts: { scan: boolean; rotate: boolean; vault: boolean; policy: string; entropyThreshold: string; format: string; output: string; maxFiles: string; maxFindings: string }) => {
      await withService(['scale', 'coherence'], async (_ctx, services) => {
        ...
        if (opts.scan) {
          ...
          if (filesToScan.length < allFiles.length) {
            output.warn(`Scanning ${filesToScan.length} of ${allFiles.length} files. Use --max-files 0 to scan all.`);
          }
          ...
          for (const file of filesToScan) {
            try {
              ...
              const found = scanForSecrets(content, file.relativePath, file.path, entropyThreshold);
              findings.push(...found);
            } catch (e) {
              logger.warn(`Skipping unreadable file in secrets scan: ${file.path} - ${e instanceof Error ? e.message : String(e)}`);
            }
          }
          ...
          if (findings.length === 0) {
            ...
            if (opts.format === 'json' || opts.format === 'sarif') {
              ...
              const content = opts.format === 'sarif' ? JSON.stringify(sarif, null, 2) : JSON.stringify({ findings }, null, 2);
              if (opts.output) {
            ...
            for (const finding of findingsToShow) {
              ...
              output.kv(`${icon} [${finding.type}] ${finding.file}:${finding.line}`, finding.maskedValue);
        ...
        if (opts.rotate) {
          ...
          for (const item of schedule.slice(0, 15)) {
            const urgent = item.daysUntilRotation <= 7 ? '🔴' : item.daysUntilRotation <= 14 ? '🟠' : '🟢';
            output.kv(`${urgent} ${item.type}`, `${item.name} - ${item.daysUntilRotation} days (${item.autoRotate ? 'auto' : 'manual'})`);
          }
        ...
        if (opts.vault) {
          ...
          if (!vaultConfig.configured) {
            ...
            output.info('To enable: Set VAULT_ADDR, VAULT_TOKEN, and configure mount paths in .projectmindrc.json');
          }
...
function scanForSecrets(content: string, relativePath: string, filePath: string, entropyThreshold: number): SecretFinding[] {
  ...
  const patterns = [
    { regex: /(?:api[_-]?key|apikey)\s*[:=]\s*["']([^"']{20,})["']/gi, type: 'api-key' as const, severity: 'high' as const },
    { regex: /(?:aws[_-]?access[_-]?key|aws[_-]?secret[_-]?key)\s*[:=]\s*["']([^"']+)["']/gi, type: 'aws-key' as const, severity: 'critical' as const },
    { regex: /(?:secret|secret[_-]?key)\s*[:=]\s*["']([^"']{20,})["']/gi, type: 'secret' as const, severity: 'high' as const },
    { regex: /(?:password|passwd|pwd)\s*[:=]\s*["']([^"']{8,})["']/gi, type: 'password' as const, severity: 'high' as const },
    { regex: /(?:token|access[_-]?token|bearer[_-]?token)\s*[:=]\s*["']([^"']{20,})["']/gi, type: 'token' as const, severity: 'high' as const },
    { regex: /-----BEGIN (?:RSA|EC|DSA|OPENSSH) PRIVATE KEY-----/g, type: 'private-key' as const, severity: 'critical' as const },
    { regex: /(?:connection[_-]?string|conn[_-]?str)\s*[:=]\s*["']([^"']+)["']/gi, type: 'connection-string' as const, severity: 'high' as const },
    { regex: /eyJ[a-zA-Z0-9_-]+\.eyJ[a-zA-Z0-9_-]+\.[a-zA-Z0-9_-]+/g, type: 'jwt' as const, severity: 'high' as const },
    { regex: /-----BEGIN CERTIFICATE-----/g, type: 'certificate' as const, severity: 'medium' as const },
  ];
...
function generateRotationSchedule(policies: Record<string, RotationPolicy>): { type: string; name: string; daysUntilRotation: number; autoRotate: boolean }[] {
  const schedule = [];
  const types = ['api-key', 'aws-key', 'private-key', 'password', 'token', 'secret', 'connection-string', 'jwt', 'certificate'];
