...
export function createMemoryCommand(): Command {
  const memoryCmd = new Command('memory')
    ...
    .action(
      asyncHandler(async (scope: string, key: string, opts: { set?: string; session?: string }) => {
        await withContext(async (ctx) => {
          if (!scope) {
            output.info('Usage: projectmind memory <scope> [key] [-s "value"] [-S session-id]');
            output.info(
              '       projectmind memory search "<natural language query>" [--limit N] [--agent name]',
            );
            return;
          }
        ...
        async (query: string, opts: { limit?: string; threshold?: string; agent?: string }) => {
          await withContext(async (ctx) => {
            ...
            for (const hit of result.hits) {
              output.kv(
                `${hit.score.toFixed(3)} [${hit.scope}/${hit.key}] by ${hit.agentName}`,
                hit.preview,
              );
            }
          });
