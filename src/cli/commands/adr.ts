...
export function createAdrCommand(): Command {
  ...
  addDirOption(
    adrCmd
      ...
      .action(asyncHandler(async (title: string, opts: { status: string; context: string; supersedes: string; dir: string }) => {
...
`;
  ...
  addDirOption(
    adrCmd
      ...
      .action(asyncHandler(async (opts: { status: string; format: string; dir: string }) => {
        const config = loadConfig();
        const adrDir = join(config.projectRoot, opts.dir);
        ...
        for (const adr of filtered) {
          output.info(`${adr.id.padEnd(3)} | ${adr.status.padEnd(12)} | ${adr.date.padEnd(10)} | ${adr.title}`);
        }
      }))
  );
  ...
  addDirOption(
    adrCmd
      ...
      .action(asyncHandler(async (opts: { output: string; dir: string }) => {
        const config = loadConfig();
        const adrDir = join(config.projectRoot, opts.dir);
