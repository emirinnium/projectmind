      ...
      asyncHandler(async (_opts: { dryRun?: boolean }) => {
        await withService(['scale'], async (_ctx, _services) => {
          ...
          for (const table of tables) {
            const count = db.prepare(`SELECT COUNT(*) as cnt FROM ${table}`).get() as {
              cnt: number;
            };
        ...
        await withService(['scale', 'debt', 'coherence'], async (ctx, services) => {
          ...
          try {
            const touched = ctx.db
              .prepare(
                'SELECT path FROM files WHERE agent_touched = 1 ORDER BY last_scanned DESC LIMIT 1',
              )
              .get() as { path?: string } | undefined;
            const anyFile =
              touched ??
              (ctx.db.prepare('SELECT path FROM files ORDER BY last_scanned DESC LIMIT 1').get() as
          ...
          if (!recentFile) {
            output.info(
              'Skipped — no files in the project database yet (run "projectmind scan" first).',
            );
          } else {
            ...
            output.kv(
              ...
              `${(historical.avgFailureRate * 100).toFixed(1)}%`,
            );
            output.kv(
              'Common broken tests',
              historical.commonBrokenTests.length > 0
                ? historical.commonBrokenTests.join(', ')
                : 'none recorded',
            );
            ...
            if (testBreaks.length > 0) {
              ...
              for (const failure of testBreaks.slice(0, 10)) {
                ...
                output.kv(
                  `  ${risk} ${failure.functionName}`,
                  `${(failure.confidence * 100).toFixed(0)}% — ${failure.reason.substring(0, 80)}`,
                );
              }
          ...
          try {
            ...
            try {
              ...
              output.warn(
                `Failed to get API at HEAD~1 for diff: ${e instanceof Error ? e.message : String(e)}`,
              );
            }
            ...
            output.warn(
              `API surface analysis failed: ${e instanceof Error ? e.message : String(e)}`,
            );
          }
          ...
          try {
            ...
            if (existsSync(trendPath)) {
              ...
              if (trendData.lastScore !== undefined) {
                ...
                output.kv(
                  `Last score`,
                  `${trendData.lastScore.toFixed(1)}% ${direction} ${change}%`,
                );
                if (Math.abs(parseFloat(change)) > 5) {
                  output.kv(
                    'Trend',
                    `Significant ${parseFloat(change) > 0 ? 'improvement' : 'decline'}`,
                  );
                }
              }
            }
          ...
          if (genome.coherenceScore > 0.85 && debtReport.bySeverity.high === 0) {
            ...
            if (genome.coherenceScore <= 0.85) {
              output.warn(
                `  - Genome score below threshold: ${(genome.coherenceScore * 100).toFixed(1)}%`,
              );
            }
          }
        });
