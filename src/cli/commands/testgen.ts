...
export function createTestgenCommand(): Command {
  const testgenCmd = new Command('testgen')
    ...
    .action(
      asyncHandler(
        async (file: string, opts: { framework: string; target: string; dryRun: boolean }) => {
...
function extractExports(
  content: string,
): Array<{ name: string; type: 'function' | 'class' | 'const' | 'interface'; params: string }> {
  const exports: Array<{
    name: string;
...
function generateTestFile(
  sourceFile: string,
  exports: Array<{ name: string; type: string; params: string }>,
  framework: string,
): string {
