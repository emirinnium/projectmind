...
export function createMigrateCommand(): Command {
  const migrateCmd = new Command('migrate')
    .description('Migration helpers for common upgrades');
  ...
  migrateCmd
    ...
    .action(asyncHandler(async (version: string) => {
      const config = loadConfig();
      ...
      if (currentTS) {
        ...
        if (targetMajor > currentMajor) {
          output.warn(`Major upgrade (${currentMajor} → ${targetMajor}) - breaking changes likely`);
