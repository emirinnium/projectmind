...
export function createWorkspaceCommand(): Command {
  return new Command('workspace')
    .description(
      'Analyze monorepo workspaces: packages, internal dependency edges, range mismatches',
    )
    ...
    .action(
      asyncHandler(async (opts: { format: string }) => {
        ...
        if (packages.length === 0) {
          output.warn(
            `No workspace packages discovered (manager: ${manager}). This does not look like a monorepo.`,
          );
          return;
        }
        ...
        interface Edge {
          ...
          to: PackageInfo;
          range: string;
          status: 'ok' | 'mismatch' | 'unknown-range';
        }
        const edges: Edge[] = [];
        ...
        const result = {
          ...
          packages: packages.map(({ dir, name, version, private: isPrivate }) => ({
            dir,
            name,
            version,
            private: !!isPrivate,
          })),
          internalEdges: edges.map((e) => ({
            from: e.from.name,
            to: e.to.name,
            range: e.range,
            status: e.status,
        ...
        for (const e of edges.sort((a, b) => a.from.name.localeCompare(b.from.name))) {
          ...
          output.kv(
            `  ${icon} ${e.from.name} → ${e.to.name}`,
            `range: ${e.range} | installed: ${e.to.version}`,
          );
        }
        ...
        if (mismatches.length > 0) {
          ...
          for (const e of mismatches) {
            output.warn(
              `${e.from.name} declares ${e.to.name}@${e.range} but installed version is ${e.to.version}`,
            );
          }
        }
      }),
