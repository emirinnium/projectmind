      ...
      asyncHandler(async (filePath: string, opts: { tests?: boolean }) => {
        await withService(['scale'], async (ctx, services) => {
          ...
          try {
            ...
            if (breaks.length > 0) {
              ...
              for (const b of breaks.slice(0, 10)) {
                output.warn(
                  `  ${b.functionName} (${Math.round(b.confidence * 100)}%): ${b.reason.substring(0, 120)}`,
                );
              }
          ...
          if (opts.tests) {
            const kg = (
              ctx as {
                kg?: { getDependents(id: number): Array<{ id: number; relativePath: string }> };
              }
            ).kg;
            if (kg && typeof targetFile.id === 'number') {
              ...
              const queue: Array<{ id: number; rel: string; depth: number }> = [
                { id: targetFile.id, rel: targetFile.relativePath, depth: 0 },
              ];
              ...
              while (queue.length > 0) {
                ...
                for (const dep of kg.getDependents(cur.id)) {
                  ...
                  visited.add(dep.id);
                  if (isTestPath(dep.relativePath))
                    impactedTests.push({ rel: dep.relativePath, depth: cur.depth + 1 });
              ...
              if (impactedTests.length === 0) {
                output.info(
                  'No tests transitively import this file — changes are not covered by the test graph.',
                );
              } else {
                for (const t of impactedTests.slice(0, 25)) {
                  output.kv(`  🧪 ${t.rel}`, `depth: ${t.depth}`);
                }
                if (impactedTests.length > 25)
                  output.info(`...and ${impactedTests.length - 25} more`);
                output.kv('Run these before committing', `${impactedTests.length} test file(s)`);
              }
            } else {
              output.info(
                '--tests requires a scanned knowledge graph (run "projectmind scan" first).',
              );
            }
          }

          // Find module containing this file
          const targetModule = report.modules.find((m) =>
            m.files?.some((f) => f.path === targetFile.path),
          );
          if (targetModule) {
          ...
          const highLoad = allFiles.filter(
            (f) => f.cognitiveLoad > 0.7 && f.path !== targetFile.path,
          );
