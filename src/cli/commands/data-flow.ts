...
export function createDataFlowCommand(): Command {
  const dataFlowCmd = new Command('data-flow').description(
  ...
  dataFlowCmd
    ...
    .requiredOption(
      '--from-kind <kind>',
      'Source resource kind: FILE|NETWORK|DATABASE|ENV|STDIN|STDOUT|STDERR|SOCKET',
    )
    ...
    .requiredOption(
      '--to-kind <kind>',
      'Target resource kind: FILE|NETWORK|DATABASE|ENV|STDIN|STDOUT|STDERR|SOCKET',
    )
        ...
        async (opts: {
          ...
          await withService(['scale'], async (ctx) => {
            ...
            const result = kg.recordDataFlow({
              ...
              fromResourceKind: opts.fromKind as
      ...
      asyncHandler(async () => {
        await withService(['scale'], async (ctx) => {
          ...
          for (const flow of flows) {
            ...
            output.kv(
              `#${flow.id} ${flow.kind}`,
              `${flow.fromResource.qualifiedName}${srcFn} -> ${flow.toResource.qualifiedName}${tgtFn}${via}`,
            );
          }
        });
      }),
  ...
  dataFlowCmd
    ...
    .action(
      asyncHandler(async (qualifiedName: string) => {
        await withService(['scale'], async (ctx) => {
          const kg = ctx.kg;
          ...
          for (const flow of flows) {
            ...
            output.kv(
              `#${flow.id} (${flow.direction})`,
              `${arrow} ${flow.resource.qualifiedName} (${flow.kind})${flow.via ? ` via ${flow.via}` : ''}`,
            );
          }
        });
      }),
