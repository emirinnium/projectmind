...
export function createAgentCommand(): Command {
  const agentCmd = new Command('agent')
    .description('Manage and inspect agent sessions and coverage');
  ...
  agentCmd
    ...
    .action(asyncHandler(async (file: string, opts: { agent: string }) => {
      await withContext(async (ctx) => {
    ...
    .action(asyncHandler(async () => {
      await withContext(async (ctx) => {
        ...
        for (const mod of report.modules) {
          ...
          output.kv(`  ${bar} ${mod.path}`, `${mod.fileCount} files, ${(mod.agentCoverage * 100).toFixed(1)}% covered`);
        }
