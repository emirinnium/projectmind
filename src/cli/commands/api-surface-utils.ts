...
export function extractExportsFromFile(
  ...
  // Functions
  const funcRegex =
    /(?:^\s*\/\*\*[\s\S]*?\*\/\s*)?export\s+(?:async\s+)?function\s+(\w+)\s*\(([^)]*)\)\s*(?::\s*([^{]+))?/gm;
  let match;
  ...
  // Classes
  const classRegex =
    /(?:^\s*\/\*\*[\s\S]*?\*\/\s*)?export\s+(?:abstract\s+)?class\s+(\w+)(?:\s+extends\s+(\w+))?(?:\s+implements\s+([^{]+))?/gm;
  ...
  const interfaceRegex =
    /(?:^\s*\/\*\*[\s\S]*?\*\/\s*)?export\s+interface\s+(\w+)(?:\s+extends\s+([^{]+))?/gm;
...
export function compareSymbols(old: ExportedSymbol, _newSym: ExportedSymbol): string[] {
  ...
  if (old.returnType !== _newSym.returnType)
    changes.push(`return type: ${old.returnType} → ${_newSym.returnType}`);
  if (old.extends !== _newSym.extends) changes.push(`extends: ${old.extends} → ${_newSym.extends}`);
  if (JSON.stringify(old.implements) !== JSON.stringify(_newSym.implements))
    changes.push(`implements changed`);
  if (old.deprecated !== _newSym.deprecated)
    changes.push(`deprecated: ${old.deprecated} → ${_newSym.deprecated}`);
