      ...
      asyncHandler(
        async (opts: {
          ...
          await withService(['scale'], async (_ctx, services) => {
            ...
            // Group by type
            const byType = currentApi.reduce(
              (acc, sym) => {
            ...
            for (const [type, symbols] of Object.entries(byType)) {
              ...
              for (const sym of symbols.slice(0, 10)) {
                ...
                output.kv(
                  `    ${sym.name}${def}${dep}`,
                  `${sym.relativePath}${sym.signature ? ` :: ${sym.signature}` : ''}`,
                );
              }
            ...
            if (diff && hasBase) {
              ...
              output.info(
                'No base reference available for diff. Use --base or --since to compare.',
              );
            }

            if (opts.output) {
              const content = JSON.stringify(
                { currentApi, baseApi: hasBase ? baseApi : null, diff },
                null,
                2,
              );
              writeFileSync(opts.output, content);
