      ...
      asyncHandler(async (opts: { since: string }) => {
        await withService(['scale'], async (_ctx, services) => {
          ...
          for (const profile of agentProfiles) {
            output.kv(
              `  ${profile.name}`,
              `${profile.filesTouched} files | ${profile.sessions} sessions | ${profile.fingerprint?.namingConvention || 'unknown'}`,
            );
          }

          output.section('File Ownership (by agent coverage)');
          const allFiles = report.modules.flatMap((m) => m.files || []);
          const touchedFiles = allFiles.filter(
            (f) =>
              f.agentTouched &&
              (!f.agentTouchedAt || new Date(f.agentTouchedAt).getTime() >= cutoff),
          );
