...
export function createStructuralSearchCommand(): Command {
  const structuralSearchCmd = new Command('structural-search')
    .description('AST-based structural search/replace across the codebase');

  structuralSearchCmd
    .command('search')
    .description('Search for AST nodes matching a pattern')
    .requiredOption('-k, --kind <kind>', 'AST node kind (e.g., FunctionDeclaration, CallExpression, IfStatement)')
    ...
    .action(asyncHandler(async (opts: {
      ...
      await withService(['scale'], async (ctx) => {
        ...
        for (const m of matches.slice(0, 50)) {
          output.kv(`${m.filePath}:${m.startLine}-${m.endLine}`, `${m.nodeKind} — ${m.text.substring(0, 120)}${m.text.length > 120 ? '...' : ''}`);
        }

        if (matches.length > 50) {
          output.info(`... and ${matches.length - 50} more matches. Use --max-results to see more.`);
        }
      });
    }));
    ...
    .action(asyncHandler(async (opts: {
      ...
      await withService(['scale'], async (ctx) => {
        ...
        if (result.dryRun) {
          ...
          output.success(`Replaced ${result.replaced} occurrences in ${result.files.length} files`);
        }
      });
    }));
