...
const DEFAULT_LAYERS: Layer[] = [
  { name: 'domain', pattern: 'src/domain/**', order: 1, description: 'Enterprise business logic, entities, value objects' },
  { name: 'application', pattern: 'src/application/**', order: 2, description: 'Use cases, application services, DTOs' },
  { name: 'infrastructure', pattern: 'src/infrastructure/**', order: 3, description: 'Database, external APIs, frameworks, UI' },
  { name: 'presentation', pattern: 'src/presentation/**', order: 4, description: 'Controllers, presenters, view models' },
  { name: 'shared', pattern: 'src/shared/**', order: 0, description: 'Shared utilities, common types, cross-cutting concerns' },
];
...
export function createLayersCommand(): Command {
  const layersCmd = new Command('layers')
    ...
    .option('--define <layers>', 'Comma-separated layer definitions (name:pattern:order:description)')
    ...
    .action(asyncHandler(async (opts: { define: string; config: string; format: string; output: string; autoFix: boolean }) => {
      await withService(['scale'], async (_ctx, services) => {
        ...
        for (const file of allFiles) {
          ...
          try {
            ...
            for (const imp of imports) {
              ...
              const importedLayerOrder = layers.find(l => l.name === importedLayer)?.order ?? 999;
        ...
        if (opts.format === 'json') {
          const result = { layers, violations: mergedViolations, summary: { totalFiles: allFiles.length, violations: mergedViolations.length, errors: mergedViolations.filter(v => v.severity === 'error').length, warnings: mergedViolations.filter(v => v.severity === 'warning').length } };
        ...
        if (mergedViolations.length === 0) {
          ...
          if (errors.length > 0) {
            ...
            for (const v of errors) {
              output.kv(`  🔴 ${v.file} (${v.layer})`, `Imports from: ${v.violatedLayers.join(', ')}`);
          ...
          if (opts.autoFix) {
            ...
            for (const v of mergedViolations.slice(0, 10)) {
              output.kv(`  ${v.file}`, `Consider moving to ${v.violatedLayers[0]} layer or using dependency inversion`);
            }
          }
        }
