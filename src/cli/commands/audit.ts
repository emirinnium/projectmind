...
export function createAuditCommand(): Command {
  return new Command('audit')
    ...
    .action(asyncHandler(async (opts: { secrets: boolean; crypto: boolean; all: boolean; format: string; maxFiles: string }) => {
      await withService(['scale', 'coherence'], async (_ctx, services) => {
        ...
        if (filesToScan.length < files.length) {
          output.warn(`Scanning ${filesToScan.length} of ${files.length} files. Use --max-files 0 to scan all.`);
        }
        
        const findings: Array<{ file: string; line: number; type: string; severity: string; message: string }> = [];
        
        // Simple secret patterns
        const secretPatterns = [
          { regex: /(api[_-]?key|secret|token|password)\s*[:=]\s*["'][^"']+["']/gi, type: 'secret', severity: 'high' },
          { regex: /(aws[_-]?access[_-]?key|aws[_-]?secret[_-]?key)\s*[:=]\s*["'][^"']+["']/gi, type: 'aws-key', severity: 'critical' },
          { regex: /-----BEGIN (RSA|EC|DSA) PRIVATE KEY-----/g, type: 'private-key', severity: 'critical' },
        ...
        if (findings.length === 0) {
          ...
          if (opts.format === 'json') {
            console.log(JSON.stringify({ findings, summary: { total: findings.length } }, null, 2));
          } else {
        ...
        const check = await coherence.checkCoherence({ code: '', filePath: 'audit', fastOnly: true });
