    ...
    .action(
      asyncHandler(async () => {
        await withContext(async () => {
          ...
          const resolved = db
            .prepare('SELECT COUNT(*) as cnt FROM imports WHERE resolved = 1')
            .get() as { cnt: number };
          ...
          const unresolved = db
            .prepare(
              'SELECT source, COUNT(*) as cnt FROM imports WHERE resolved = 0 GROUP BY source ORDER BY cnt DESC LIMIT 10',
            )
    ...
    .action(
      asyncHandler(async () => {
        await withContext(async () => {
          ...
          output.kv(
            'Total size',
            `${((pageSize.page_size * pageCount.page_count) / 1024 / 1024).toFixed(2)} MB`,
          );
        });
      }),
    );
      ...
      asyncHandler(async () => {
        await withContext(async (ctx) => {
          ...
          for (const f of files.slice(0, 20)) {
            output.kv(
              `  ${f.relativePath}`,
              `by=${f.agentTouchedBy}, at=${f.agentTouchedAt}, load=${f.cognitiveLoad.toFixed(3)}`,
            );
          }
        });
      }),
