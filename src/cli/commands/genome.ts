...
export function createGenomeCommand(): Command {
  return new Command('genome').description('Compute and display project coherence genome').action(
    asyncHandler(async () => {
      await withService(['debt'], async (_ctx, services) => {
        ...
        try {
          ...
          if (typeof b.avgConfidence === 'number')
            output.kv('Avg pattern confidence', `${Math.round(b.avgConfidence * 100)}%`);
          if (typeof b.highConfidencePatterns === 'number')
            output.kv('High-confidence patterns', String(b.highConfidencePatterns));
          if (typeof b.importResolutionRate === 'number')
            output.kv('Import resolution rate', `${Math.round(b.importResolutionRate * 100)}%`);
          if (b.circularDepPenalty)
            output.warn(`Circular dependency penalty applied: ${b.circularDepPenalty}`);
