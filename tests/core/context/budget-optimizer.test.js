...
import { ContextBudgetOptimizer, applyTaskTypeBoosts, } from '../../../src/core/context/budget-optimizer.js';
import { greedySelector, dpSelector, dpApplicable, DP_MAX_ITEMS, } from '../../../src/core/context/knapsack.js';
describe('ContextBudgetOptimizer', () => {
    ...
    it('task-type boost changes selection for bug fix tasks', () => {
        ...
        const x = { path: 'x.ts', tokens: 5, relevanceScore: 0.5, recentlyChanged: true };
        const y = { path: 'y.ts', tokens: 5, relevanceScore: 0.51 };
        const opt = new ContextBudgetOptimizer();
    ...
    it('test task type boosts test files', () => {
        const testFile = { path: 'a.test.ts', tokens: 5, relevanceScore: 0.5, isTestFile: true };
        const srcFile = { path: 'a.ts', tokens: 5, relevanceScore: 0.51 };
    ...
    it('falls back to greedy when n exceeds DP bounds and respects budget', () => {
        const many = Array.from({ length: DP_MAX_ITEMS + 5 }, (_, i) => ({
            path: `m-${i}.ts`,
            tokens: 10 + (i % 7),
    ...
    it('end-to-end: task-type boost >1 beats a higher-base unboosted item', () => {
        ...
        const plain = { path: 'plain.ts', tokens: 10, relevanceScore: 1.0 };
    ...
    it('OOM-bound sanity: n=60 all at the 2.0 clamp completes within bounds', () => {
        const items = Array.from({ length: DP_MAX_ITEMS }, (_, i) => ({
            path: `f-${i}.ts`,
            tokens: 100 + i,
            relevanceScore: 2.0,
        }));
...
//# sourceMappingURL=budget-optimizer.test.js.map