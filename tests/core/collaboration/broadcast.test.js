...
describe('IntentBroadcastService', () => {
    ...
    it('ignores expired intents in checkConflict and expireIntents removes only expired', async () => {
        ...
        const rows = db.prepare('SELECT target_files FROM pending_intents').all();
        const allFiles = rows.flatMap((r) => JSON.parse(r.target_files));
    ...
    it('does not persist intents from private branches and marks scope local', () => {
        ...
        const rows = db.prepare('SELECT COUNT(*) AS n FROM pending_intents').get();
    ...
    it('persists intents from public branches with shared scope', () => {
        ...
        const rows = db.prepare('SELECT target_files FROM pending_intents').all();
        expect(rows.flatMap((r) => JSON.parse(r.target_files))).toContain('public-work.ts');
    });
    ...
    it('stores broadcast_at and expires_at as integer unix milliseconds (F20)', () => {
        ...
        const row = db.prepare('SELECT broadcast_at, expires_at FROM pending_intents').get();
        ...
        expect(row.expires_at).toBeGreaterThanOrEqual(row.broadcast_at + 119_000);
        expect(row.expires_at).toBeLessThanOrEqual(row.broadcast_at + 121_000);
    });
    ...
    describe('poisoned target_files rows (valid JSON, not string[])', () => {
        ...
        function insertPoisonRows() {
            ...
            POISONS.forEach((payload, i) => {
                db.prepare(`INSERT INTO pending_intents (agent_id, intent_type, target_files, broadcast_at, expires_at)
           VALUES (?, ?, ?, ?, ?)`).run(`poison-${i}`, 'write', payload, now, now + 300_000);
            });
        }
        function insertGenuineRow(agentId, file) {
            const now = Date.now();
            db.prepare(`INSERT INTO pending_intents (agent_id, intent_type, target_files, broadcast_at, expires_at)
         VALUES (?, ?, ?, ?, ?)`).run(agentId, 'write', JSON.stringify([file]), now, now + 300_000);
        }
        ...
        it('getActiveIntents returns genuine rows after poison rows', () => {
            ...
            expect(active.every((b) => Array.isArray(b.targetFiles) && b.targetFiles.every((f) => typeof f === 'string'))).toBe(true);
        });
        ...
        it('non-object expected_changes payloads are discarded, intent kept', () => {
            const now = Date.now();
            db.prepare(`INSERT INTO pending_intents (agent_id, intent_type, target_files, expected_changes, broadcast_at, expires_at)
         VALUES (?, ?, ?, ?, ?, ?)`).run('agent-ec', 'write', '["src/ec.ts"]', '"just-a-string"', now, now + 300_000);
            const fresh = new IntentBroadcastService(db, { branchDetector: () => 'main' });
            const active = fresh.getActiveIntents('agent-b');
