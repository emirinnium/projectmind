...
import { IntentEngine, classifyTask } from '../../../src/core/search/intent-engine.js';
...
describe('IntentEngine WP1', () => {
    ...
    describe('F1 backward compat and spec', () => {
        it('deprecated text alias works', () => {
            const q = { text: 'find auth' };
            expect(engine.classifyIntent(q)).toBe('read');
        });
        it('naturalLanguage + structuralHints + expectedOutputs accepted', () => {
            const q = { naturalLanguage: 'validate login', structuralHints: ['auth'], expectedOutputs: ['test'] };
    ...
    describe('path traversal guard (projectRoot containment)', () => {
        ...
        beforeAll(() => {
            ...
            writeFileSync(join(parent, 'SECRET.txt'), 'readFileSync(secret); readFileSync(secret); readFileSync(secret);');
        ...
        it('search snippets never leak content from outside projectRoot', async () => {
            const rooted = new IntentEngine({ projectRoot: root });
            const results = await rooted.search({ naturalLanguage: 'read the secret', filePath: '../SECRET.txt' }, { getFileByPath: () => null }, 5);
...
//# sourceMappingURL=intent-engine.test.js.map