...
describe('ImpactPredictor WP2', () => {
    ...
    describe('F6 recordOutcome -> correlateHistoricalFailures', () => {
        it('persists file_path and correlates', () => {
            ...
            const impact = { predictionId: 'p1', filePath: 'src/auth.ts', actualAffectedFiles: 2, actualAffectedModules: ['auth'], failureOccurred: true, severity: 'high' };
    ...
    describe('F7 call-site analysis', () => {
        it('flags stale mock after arity change', () => {
            ...
            const change = { filePath: srcFile, moduleName: 'src', changeType: 'modify', crossModule: false, affectedFunctions: ['foo'] };
    ...
    describe('F8 git fallback', () => {
        it('does not throw when no git info', () => {
            const predictor = new ImpactPredictor(config);
            const change = { filePath: '/nonexistent/file.ts', moduleName: 'x', changeType: 'modify', crossModule: false };
    ...
    describe('F10 PredictedFailure shape', () => {
        it('asserts PredictedFailure fields', () => {
            const failure = { filePath: 'f.ts', functionName: 'fn', confidence: 0.8, reason: 'r', suggestedFix: 'fix' };
...
//# sourceMappingURL=impact-predictor.test.js.map