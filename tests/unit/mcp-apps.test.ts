...
function makeScaleReport(): ScaleReport {
  return {
    totalFiles: 3,
    totalBytes: 300,
    totalLines: 30,
...
function makeDebtReport(): DebtReport {
  return {
    totalItems: 10,
...
describe('apps content envelope', () => {
  it('buildAppsPayload wraps components in the mcp-apps v1 envelope', () => {
    ...
    const parsed = JSON.parse(payload) as { 'mcp-apps': string; components: AppComponent[] };
