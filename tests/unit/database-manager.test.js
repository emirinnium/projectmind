...
describe('DatabaseManager', () => {
    ...
    describe('init', () => {
        ...
        it('configures SQLite pragmas', () => {
            ...
            const fkResult = manager.getDb().prepare('PRAGMA foreign_keys').get();
            expect(fkResult.foreign_keys).toBe(1);
        });
        it('creates schema tables', () => {
            manager = new DatabaseManager();
            manager.init();
            const tables = manager.getDb().prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='files'").get();
    ...
    describe('runInTransaction', () => {
        ...
        it('supports nested transactions', () => {
            ...
            const outer = manager.getDb().prepare('SELECT * FROM projects WHERE name = ?').get('outer');
            const inner = manager.getDb().prepare('SELECT * FROM projects WHERE name = ?').get('inner');
    ...
    describe('busy_timeout (SQLITE_BUSY mitigation)', () => {
        it('DatabaseManager.init() sets busy_timeout to 5000ms', () => {
            manager = new DatabaseManager();
            manager.init();
            const row = manager.getDb().prepare('PRAGMA busy_timeout').get();
...
//# sourceMappingURL=database-manager.test.js.map