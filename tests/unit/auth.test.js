...
describe('ClientRegistry — RFC 7591 dynamic client registration', () => {
    ...
    it('rejects non-https / non-loopback redirect_uris', () => {
        const reg = new ClientRegistry(db);
        expect(() => reg.register({ client_name: 'Bad', redirect_uris: ['http://evil.example.com/cb'] })).toThrowError(AuthError);
        expect(() => reg.register({ client_name: 'Bad', redirect_uris: ['javascript:alert(1)'] })).toThrowError(AuthError);
    });
    ...
    it('rejects client_credentials with auth method "none"', () => {
        const reg = new ClientRegistry(db);
        expect(() => reg.register({ grant_types: ['client_credentials'], token_endpoint_auth_method: 'none' })).toThrowError(/client_credentials/);
    });
...
describe('handleOauthRoute — HTTP surface', () => {
    ...
    it('registers a client over HTTP (201 + Location)', () => {
        const c = ctx();
        const result = handleOauthRoute('/oauth/register', JSON.stringify({ client_name: 'web', redirect_uris: ['https://app.example.com/cb'] }), 'application/json', c);
    ...
    it('rejects a bad registration with an RFC 7591 error body', () => {
        const c = ctx();
        const result = handleOauthRoute('/oauth/register', JSON.stringify({ redirect_uris: ['http://evil.com/cb'] }), 'application/json', c);
        expect(result.handled).toBe(true);
        if (!result.handled)
            return;
    ...
    it('issues a client_credentials token (secret_post)', () => {
        ...
        const result = handleOauthRoute('/oauth/token', JSON.stringify({
        ...
        expect(c.tokens.verify(result.payload.access_token).clientId).toBe(client.client_id);
    });
    it('accepts client_secret_basic on the token endpoint', () => {
        ...
        const result = handleOauthRoute('/oauth/token', JSON.stringify({ grant_type: 'client_credentials' }), 'application/json', { registry: reg, tokens: new TokenService(db), authorization: `Basic ${basic}` });
        expect(result.handled).toBe(true);
        if (!result.handled)
            return;
        expect(result.status).toBe(200);
    ...
    it('rejects bad credentials with invalid_client', () => {
        ...
        const result = handleOauthRoute('/oauth/token', JSON.stringify({ grant_type: 'client_credentials', client_id: client.client_id, client_secret: process.env.SECRET_WRONG }), 'application/json', ctx({ registry: reg }));
        expect(result.handled).toBe(true);
        if (!result.handled)
            return;
        expect(result.status).toBe(400);
    ...
    describe('scope allowlist (allowedScopes)', () => {
        ...
        it('grants an authorized requested scope', () => {
            ...
            }), 'application/json', { ...ctx({ registry: reg }), allowedScopes: ['projectmind:mcp'] });
            expect(result.handled).toBe(true);
            if (!result.handled)
                return;
            expect(result.status).toBe(200);
...
describe('SQLite persistence — OAuth data survives service restarts', () => {
    ...
    it('rejects expired tokens via the SQL WHERE expires_at clause', () => {
        ...
        const row = db
            ...
            .get(hashToken(res.access_token));
    ...
    it('persists only the sha256 token hash — plaintext is NEVER written (K6)', () => {
        ...
        const rows = db.prepare('SELECT COUNT(*) AS n FROM oauth_tokens WHERE token = ?').get(res.access_token);
