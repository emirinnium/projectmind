...
describe('writeOpencodeConfig', () => {
  it('writes a spec-compliant opencode.json at the project root', () => {
    ...
    expect(result.path).toBe(full);
    const parsed = JSON.parse(readFileSync(full, 'utf-8')) as Record<string, unknown>;
    expect(parsed.$schema).toBe('https://opencode.ai/config.json');
    const mcp = parsed.mcp as Record<string, { type: string; enabled?: boolean }>;
  ...
  it('merges with an existing opencode.json instead of destroying it', () => {
    ...
    const result = writeOpencodeConfig(FIXTURE_DIR, true);
    expect(result.written).toBe(true);
    const parsed = JSON.parse(readFileSync(join(FIXTURE_DIR, OPENCODE_CONFIG_FILENAME), 'utf-8')) as Record<string, unknown>;
    expect(parsed.model).toBe('anthropic/claude-sonnet-4'); // preserved
    const mcp = parsed.mcp as Record<string, unknown>;
    expect((mcp.projectmind as { enabled: boolean }).enabled).toBe(true);
  });
});
