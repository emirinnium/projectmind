...
describe('Project Operations', () => {
    ...
    it('inserts and retrieves a project', () => {
        ...
        const row = db.prepare('SELECT * FROM projects WHERE name = ?').get('test-project');
...
describe('File Operations', () => {
    ...
    it('inserts and retrieves a file', () => {
        ...
        const row = db.prepare('SELECT * FROM files WHERE path = ?').get('/test/src/index.ts');
        expect(row).toBeDefined();
    ...
    it('upserts a file (insert then update)', () => {
        ...
        const row = db.prepare('SELECT * FROM files WHERE path = ?').get('/test/file.ts');
    ...
    it('stores and retrieves functions for a file', () => {
        ...
        const fileId = db.prepare('SELECT id FROM files WHERE path = ?').get('/test/file.ts').id;
        ...
        const fns = db.prepare('SELECT * FROM functions WHERE file_id = ?').all(fileId);
    ...
    it('stores and retrieves classes for a file', () => {
        ...
        const classes = db.prepare('SELECT * FROM classes WHERE file_id = ?').all(fileId);
    ...
    it('stores and retrieves imports for a file', () => {
        ...
        const imports = db.prepare('SELECT * FROM imports WHERE file_id = ?').all(fileId);
...
describe('Resource & Data Flow Operations', () => {
    ...
    it('records data flows between resources', () => {
        ...
        const flows = db.prepare('SELECT * FROM data_flows WHERE project_id = ?').all(1);
...
describe('Settings Operations', () => {
    ...
    it('stores and retrieves settings', () => {
        ...
        const row = db.prepare('SELECT * FROM settings WHERE key = ?').get('current_project_id');
...
describe('KnowledgeGraph — dynamic calls + path lookup hardening', () => {
    ...
    describe('ingestDynamicCalls — ensureFunction never fabricates rows', () => {
        it('unknown function names insert nothing and report errors', async () => {
            ...
            const before = db.prepare('SELECT COUNT(*) AS n FROM functions').get().n;
            ...
            const after = db.prepare('SELECT COUNT(*) AS n FROM functions').get().n;
...
//# sourceMappingURL=knowledge-graph.test.js.map