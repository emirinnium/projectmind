...
describe('extractCodebaseSkills — repo evidence mapping', () => {
  it('maps real module paths onto skills and drops un-evidenced ones', () => {
    const evidence = extractCodebaseSkills(
      moduleWith(
        'src/parser/structural-search.ts',
        'src/parser/structural-search.ts',
        'src/core/debt/tracker.ts',
        'src/vendor/opaque.js',
      ),
    );
...
describe('estimateSkillProficiency — interaction-history evidence', () => {
  it('stays 0 with zero evidence (never fabricated)', () => {
    expect(
      estimateSkillProficiency(
        SKILL_CATALOG.find((s) => s.id === 'ast-parsing')!,
        {
          sessionCount: 0,
          touchedPaths: [],
          decisionsText: '',
  ...
  it('adds 0.35 per touched file matching the skill signals', () => {
    ...
    const many = estimateSkillProficiency(skill, {
      sessionCount: 0,
      touchedPaths: ['src/parser/a.ts', 'src/parser/b.ts', 'src/parser/c.ts', 'src/parser/d.ts'],
      decisionsText: '',
      asyncPreference: -1,
    });
  ...
  it('boosts async-patterns from the async fingerprint', () => {
    ...
    const withFp = estimateSkillProficiency(skill, {
      sessionCount: 0,
      touchedPaths: [],
      decisionsText: '',
      asyncPreference: 0.9,
    });
    const withoutFp = estimateSkillProficiency(skill, {
      sessionCount: 0,
      touchedPaths: [],
      decisionsText: '',
      asyncPreference: -1,
  ...
  it('estimates every catalog skill within 0..1', () => {
    const all = estimateAllProficiencies({
      sessionCount: 1,
      touchedPaths: ['src/mcp-server.ts'],
      decisionsText: 'x',
      asyncPreference: 0.5,
    });
...
describe('analyzeSkillGaps', () => {
  it('reports gaps above the threshold, sorted by size', () => {
    const gaps = analyzeSkillGaps(
      { 'ast-parsing': 0, 'mcp-protocol': 1 },
      evidenceWith('ast-parsing', 'mcp-protocol'),
      0.3,
    );
...
describe('generateSkillDoc — personalized SKILL.md', () => {
  const gaps = analyzeSkillGaps(
    { 'ast-parsing': 0 },
    evidenceWith('ast-parsing', 'typescript'),
    0.1,
  );
