...
function createTestDbWithoutMigrations() {
...
describe('Migration Rollback', () => {
    ...
    describe('setSchemaVersion', () => {
        ...
        it('replaces existing version with same number', () => {
            ...
            const row = db.prepare('SELECT name FROM schema_version WHERE version = ?').get(1);
...
describe('Legacy DB upgrades', () => {
    ...
    it('upgrades a v6 DB to the latest schema without error', () => {
        ...
        const row = db.prepare('SELECT last_synced, last_scanned FROM files WHERE id = 1').get();
        ...
        const count = db.prepare('SELECT COUNT(*) as c FROM debt_items').get();
    ...
    `);
        ...
        runMigrations(db);
        const leftover = db.prepare('SELECT COUNT(*) AS n FROM oauth_tokens').get();
    ...
    `);
        ...
        runMigrations(db);
        const left = db.prepare('SELECT COUNT(*) AS n FROM circular_dependencies').get();
        ...
        expect(db.prepare('SELECT COUNT(*) AS n FROM circular_dependencies').get().n).toBe(2);
        db.close();
    });
});
describe('Migration 93 — pending_intents unix-ms reconciliation (F40)', () => {
    ...
    function tableInfo(db) {
        return db.prepare('PRAGMA table_info(pending_intents)').all();
    }
    /**
  ...
  `;
    ...
    `);
        db.prepare('INSERT INTO pending_intents (id, agent_id, intent_type, target_files, expected_changes, timestamp, ttl_seconds) VALUES (?, ?, ?, ?, ?, ?, ?)').run('legacy-uuid-1', 'agent-a', 'write', '["src/a.ts"]', '{"notes":["change sig"]}', 1000, 300);
        db.prepare('INSERT INTO pending_intents (id, agent_id, intent_type, target_files, expected_changes, timestamp, ttl_seconds) VALUES (?, ?, ?, ?, ?, ?, ?)').run('legacy-uuid-2', 'agent-b', 'refactor', null, null, null, null);
    ...
    it('is a no-op on a fresh canonical database', () => {
        ...
        db.prepare(`INSERT INTO pending_intents (agent_id, intent_type, target_files, broadcast_at, expires_at) VALUES (?, ?, ?, ?, ?)`).run('agent-c', 'read', '["src/c.ts"]', 5000, 305000);
        setSchemaVersion(db, 92, 'add_pattern_origin_and_collaboration');
        runMigrations(db);
        expect(getCurrentSchemaVersion(db)).toBe(95);
        const row = db
    ...
    `);
        ...
        db.prepare('INSERT INTO pending_intents (agent_id, intent_type, target_files, broadcast_at, expires_at) VALUES (?, ?, ?, ?, ?)').run('agent-d', 'write', '["src/d.ts"]', '2025-12-31 23:55:00', '2026-01-01 00:00:00');
        db.prepare('INSERT INTO pending_intents (agent_id, intent_type, target_files, broadcast_at, expires_at) VALUES (?, ?, ?, ?, ?)').run('agent-e', 'refactor', '["src/e.ts"]', '2025-12-31T23:55:00.000Z', '2026-01-01T00:00:00Z');
        db.prepare('INSERT INTO pending_intents (agent_id, intent_type, target_files, broadcast_at, expires_at) VALUES (?, ?, ?, ?, ?)').run('agent-f', 'delete', '["src/f.ts"]', 'not-a-timestamp', 'garbage-value');
        setSchemaVersion(db, 92, 'add_pattern_origin_and_collaboration');
        runMigrations(db);
        expect(getCurrentSchemaVersion(db)).toBe(95);
        const cols = tableInfo(db);
        ...
        const rowD = db.prepare('SELECT * FROM pending_intents WHERE agent_id = ?').get('agent-d');
        ...
        const rowE = db.prepare('SELECT * FROM pending_intents WHERE agent_id = ?').get('agent-e');
        expect(rowE.expires_at).toBe(expectedMs);
        // Unparseable strings → 0 (treated as expired, never crash).
        const rowF = db.prepare('SELECT * FROM pending_intents WHERE agent_id = ?').get('agent-f');
    ...
    `);
        ...
        db.exec(`INSERT INTO pending_intents (agent_id, intent_type, target_files, broadcast_at, expires_at)
       VALUES ('agent-g', 'write', '["src/g.ts"]', '2025-12-31 23:55:00', '2026-01-01 00:00:00')`);
        setSchemaVersion(db, 92, 'add_pattern_origin_and_collaboration');
        runMigrations(db);
        expect(getCurrentSchemaVersion(db)).toBe(95);
        const row = db.prepare('SELECT * FROM pending_intents WHERE agent_id = ?').get('agent-g');
    ...
    `);
        ...
        db.prepare('INSERT INTO pending_intents (agent_id, intent_type, target_files, broadcast_at, expires_at) VALUES (?, ?, ?, ?, ?)').run('agent-sec', 'write', '["src/s.ts"]', '1767225600', '1767225900');
        // 13-digit numeric TEXT → already ms, must pass through unchanged.
        db.prepare('INSERT INTO pending_intents (agent_id, intent_type, target_files, broadcast_at, expires_at) VALUES (?, ?, ?, ?, ?)').run('agent-ms', 'refactor', '["src/m.ts"]', '1767225600000', '1767225900000');
        setSchemaVersion(db, 92, 'add_pattern_origin_and_collaboration');
        runMigrations(db);
        expect(getCurrentSchemaVersion(db)).toBe(95);
        const rowSec = db.prepare('SELECT * FROM pending_intents WHERE agent_id = ?').get('agent-sec');
        expect(rowSec.broadcast_at).toBe(1767225600 * 1000);
        expect(rowSec.expires_at).toBe(1767225900 * 1000);
        const rowMs = db.prepare('SELECT * FROM pending_intents WHERE agent_id = ?').get('agent-ms');
    ...
    it('converts the genuine shipped schema.ts shape (NO expected_changes) without losing rows', () => {
        ...
        db.prepare('INSERT INTO pending_intents (agent_id, intent_type, target_files, session_id, description, broadcast_at, expires_at) VALUES (?, ?, ?, ?, ?, ?, ?)').run('agent-old-1', 'write', '["src/old1.ts"]', 'sess-1', 'legacy row one', '2025-12-31 23:55:00', '2026-01-01 00:00:00');
        db.prepare('INSERT INTO pending_intents (agent_id, intent_type, target_files, session_id, description, broadcast_at, expires_at) VALUES (?, ?, ?, ?, ?, ?, ?)').run('agent-old-2', 'read', '["src/old2.ts"]', null, null, '2025-12-31 23:56:00', '2026-01-01 00:01:00');
        setSchemaVersion(db, 92, 'add_pattern_origin_and_collaboration');
        runMigrations(db);
        expect(getCurrentSchemaVersion(db)).toBe(95);
        const cols = tableInfo(db);
        ...
        const row1 = db.prepare('SELECT * FROM pending_intents WHERE agent_id = ?').get('agent-old-1');
        ...
        const row2 = db.prepare('SELECT * FROM pending_intents WHERE agent_id = ?').get('agent-old-2');
        expect(row2.expected_changes).toBeNull();
        expect(row2.expires_at).toBe(Date.parse('2026-01-01T00:01:00Z'));
        expect(db.prepare('SELECT COUNT(*) AS n FROM pending_intents').get().n).toBe(2);
        db.close();
    });
    it('runs v92 else-branch + v93 on the genuine old shape (populated table)', () => {
        ...
        const db = new DatabaseSync(':memory:');
        db.exec(GENUINE_OLD_SHAPE_DDL);
        db.prepare('INSERT INTO pending_intents (agent_id, intent_type, target_files, session_id, description, broadcast_at, expires_at) VALUES (?, ?, ?, ?, ?, ?, ?)').run('agent-combo', 'write', '["src/combo.ts"]', null, null, '2025-12-31 23:55:00', '2026-01-01 00:00:00');
        setSchemaVersion(db, 11, 'circular_dependencies_unique');
        expect(() => runMigrations(db)).not.toThrow();
        expect(getCurrentSchemaVersion(db)).toBe(95);
        expect(tableInfo(db).map((c) => c.name).sort()).toEqual([...CANONICAL_COLUMNS].sort());
        const row = db.prepare('SELECT * FROM pending_intents WHERE agent_id = ?').get('agent-combo');
    ...
    it('converts a populated genuine v92 table created by migration 92 itself', () => {
        ...
        db.prepare('INSERT INTO pending_intents (id, agent_id, intent_type, target_files, expected_changes, timestamp, ttl_seconds) VALUES (?, ?, ?, ?, ?, ?, ?)').run('genuine-v92-1', 'agent-v92', 'write', '["src/v92.ts"]', '{"add":["param"]}', 1767225600, 300);
        runMigrations(db); // only v93 + v94 pending
        expect(getCurrentSchemaVersion(db)).toBe(95);
        expect(tableInfo(db).map((c) => c.name).sort()).toEqual([...CANONICAL_COLUMNS].sort());
        const row = db.prepare('SELECT * FROM pending_intents WHERE agent_id = ?').get('agent-v92');
    ...
    it('handles an empty genuine old-shape table', () => {
        ...
        expect(getCurrentSchemaVersion(db)).toBe(95);
        expect(tableInfo(db).map((c) => c.name).sort()).toEqual([...CANONICAL_COLUMNS].sort());
        expect(db.prepare('SELECT COUNT(*) AS n FROM pending_intents').get().n).toBe(0);
        db.close();
    });
    ...
    `);
        ...
        runMigrations(db);
        expect(getCurrentSchemaVersion(db)).toBe(95);
        const rows = db.prepare('SELECT id, agent_id, expected_changes FROM pending_intents ORDER BY agent_id').all();
    ...
    it('maps unparseable legacy datetime values to 0 (expired) on the genuine old shape', () => {
        const db = new DatabaseSync(':memory:');
        db.exec(GENUINE_OLD_SHAPE_DDL);
        db.prepare('INSERT INTO pending_intents (agent_id, intent_type, target_files, broadcast_at, expires_at) VALUES (?, ?, ?, ?, ?)').run('agent-garbage', 'delete', '["src/garbage.ts"]', 'not-a-timestamp', 'garbage-value');
        setSchemaVersion(db, 92, 'add_pattern_origin_and_collaboration');
        runMigrations(db);
        expect(getCurrentSchemaVersion(db)).toBe(95);
        const row = db.prepare('SELECT * FROM pending_intents WHERE agent_id = ?').get('agent-garbage');
    ...
    `);
        db.prepare('INSERT INTO pending_intents (agent_id, intent_type, target_files, broadcast_at, expires_at) VALUES (?, ?, ?, ?, ?)').run(null, 'write', '["src/junk.ts"]', '2025-12-31 23:55:00', '2026-01-01 00:00:00');
        db.prepare('INSERT INTO pending_intents (agent_id, intent_type, target_files, broadcast_at, expires_at) VALUES (?, ?, ?, ?, ?)').run('agent-valid', 'write', '["src/valid.ts"]', '2025-12-31 23:55:00', '2026-01-01 00:00:00');
        setSchemaVersion(db, 92, 'add_pattern_origin_and_collaboration');
        expect(() => runMigrations(db)).not.toThrow();
        expect(getCurrentSchemaVersion(db)).toBe(95);
        const rows = db.prepare('SELECT agent_id FROM pending_intents').all();
    ...
    it('v93 canonical no-op removes a stray legacy timestamp column and its index', () => {
        ...
        db.prepare('INSERT INTO pending_intents (agent_id, intent_type, target_files, broadcast_at, expires_at, timestamp) VALUES (?, ?, ?, ?, ?, ?)').run('agent-stray', 'write', '["src/stray.ts"]', 5000, 305000, 5);
        setSchemaVersion(db, 92, 'add_pattern_origin_and_collaboration');
        runMigrations(db);
        expect(getCurrentSchemaVersion(db)).toBe(95);
        expect(tableInfo(db).map((c) => c.name).sort()).toEqual([...CANONICAL_COLUMNS].sort());
        const row = db.prepare('SELECT * FROM pending_intents WHERE agent_id = ?').get('agent-stray');
    ...
    `);
        db.exec(`INSERT INTO pending_intents (agent_id, intent_type, target_files, expires_at)
       VALUES ('agent-nb', 'write', '["src/nb.ts"]', '2026-01-01 00:00:00')`);
        ...
        const row = db.prepare('SELECT * FROM pending_intents WHERE agent_id = ?').get('agent-nb');
...
describe('Migration 94 — debt_items change_frequency + project_id backfill', () => {
    ...
    `);
        ...
        const legacy = db.prepare('SELECT description FROM debt_items WHERE type = ?').get('complexity');
    ...
    it('backfills project_id on files and data_flows of a pre-existing DB', () => {
        ...
        const dataFlowsCols = db.prepare('PRAGMA table_info(data_flows)').all();
...
describe('Migration 95 — FK cascades + imports resolved_path index', () => {
    ...
    function seedParentAndChildren(db) {
        ...
        const fileId = Number(db.prepare('SELECT id FROM files WHERE path = ?').get('src/gone.ts').id);
        const patternId = Number(db.prepare('SELECT id FROM patterns WHERE name = ?').get('p1').id);
    ...
    it('legacy v94 DB: deleting a file cascades to violations/decisions/debt after migration 95', () => {
        ...
        expect(db.prepare('SELECT COUNT(*) AS n FROM pattern_violations').get().n).toBe(1);
        expect(db.prepare('SELECT COUNT(*) AS n FROM coherence_decisions').get().n).toBe(1);
        expect(db.prepare('SELECT COUNT(*) AS n FROM debt_items').get().n).toBe(1);
        db.prepare('DELETE FROM files WHERE id = ?').run(fileId);
        expect(db.prepare('SELECT COUNT(*) AS n FROM pattern_violations WHERE file_id = ?').get(fileId).n).toBe(0);
        expect(db.prepare('SELECT COUNT(*) AS n FROM coherence_decisions WHERE file_id = ?').get(fileId).n).toBe(0);
        expect(db.prepare('SELECT COUNT(*) AS n FROM debt_items WHERE file_id = ?').get(fileId).n).toBe(0);
        ...
        const keptFileId = Number(db.prepare('SELECT id FROM files WHERE path = ?').get('src/kept.ts').id);
        ...
        expect(db.prepare('SELECT COUNT(*) AS n FROM pattern_violations WHERE pattern_id = ?').get(patternId).n).toBe(0);
        db.close();
    });
