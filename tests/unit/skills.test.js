...
import { SKILL_CATALOG, analyzeSkillGaps, estimateAllProficiencies, estimateSkillProficiency, extractCodebaseSkills, generateSkillDoc, } from '../../src/core/skills/engine.js';
function moduleWith(...paths) {
    return [
        {
            path: 'mock',
            name: 'mock',
...
describe('extractCodebaseSkills — repo evidence mapping', () => {
    it('maps real module paths onto skills and drops un-evidenced ones', () => {
        const evidence = extractCodebaseSkills(moduleWith('src/parser/structural-search.ts', 'src/parser/structural-search.ts', 'src/core/debt/tracker.ts', 'src/vendor/opaque.js'));
    ...
    it('never lists the same file twice within a single skill (typescript special branch + indicator loop)', () => {
        const evidence = extractCodebaseSkills(moduleWith('src/parser/structural-search.ts', 'src/parser/x.ts', 'vitest.config.ts', 'scripts/generate-logos.ts'));
...
describe('estimateSkillProficiency — interaction-history evidence', () => {
    it('stays 0 with zero evidence (never fabricated)', () => {
        expect(estimateSkillProficiency(SKILL_CATALOG.find((s) => s.id === 'ast-parsing'), {
            sessionCount: 0,
            touchedPaths: [],
            decisionsText: '',
            asyncPreference: -1,
