...
describe('StructuralSearcher - search', () => {
    it('finds async function declarations', () => {
        const matches = searcher.search({ nodeKind: 'FunctionDeclaration', hasModifier: 'async', maxResults: 10 }, [TS_FILE]);
    ...
    it('finds functions by name pattern', () => {
        const matches = searcher.search({ nodeKind: 'FunctionDeclaration', namePattern: '^add' }, [TS_FILE]);
        expect(matches.length).toBe(1);
    ...
    it('respects maxResults limit', () => {
        const matches = searcher.search({ nodeKind: 'FunctionDeclaration', maxResults: 2 }, [TS_FILE, NESTED_FILE]);
        expect(matches.length).toBeLessThanOrEqual(2);
    });
    it('returns empty array when nothing matches', () => {
        const matches = searcher.search({ nodeKind: 'FunctionDeclaration', namePattern: '^nonexistent' }, [TS_FILE]);
...
//# sourceMappingURL=structural-search.test.js.map